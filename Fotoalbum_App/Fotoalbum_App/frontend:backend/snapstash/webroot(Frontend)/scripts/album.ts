 ...
 * @property {number} photo_id - the ID for the photo.
 * @property {number} album_id - The ID of the album to which this photo belongs.
 * @property {string} creation_date - The date when the photo was uploaded.
 * @property {string} url - The URL where the photo image is accessible.
 */
interface Photo {
  title: string;
  photo_id: number;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  //Show Album Keywords
  const albumKeywords = await loadKeywordsFromAlbum(parseInt(album_id));
    ...
    searchBtn.addEventListener('click', async () => {
      ...
      if (searched_stringInput) {
        ...
        if (searched_string) {
          const container = document.getElementById("photos");
          container.innerHTML = null;
          await searchPhoto(searched_string);
        }
      } else {
  ...
  if (saveChangesBtn) {
    saveChangesBtn.addEventListener('click', async () => {
      ...
      const photoTitleInput = document.getElementById('photo-edit-title') as HTMLInputElement;
      const photoCreationInput = document.getElementById('photo-creationDate') as HTMLInputElement;
      const hiddenPhotoIdInput = document.getElementById('photo-edit-id') as HTMLInputElement;
      const photoKeywordsInput = document.getElementById('photo-edit-keywords') as HTMLInputElement; //optional
      if (photoTitleInput && photoCreationInput && photoKeywordsInput) {
        ...
        if (photoTitle && hiddenPhotoIdInput && photoKeywordsInput) {
          await saveChanges(photoTitle, parseInt(hiddenPhotoIdInput.value), photoCreationInput.value, photoKeywords);
        } else {
  ...
  if (addPhotoBtn) {
    addPhotoBtn.addEventListener('click', async () => {
      ...
      const photoTitleInput = document.getElementById('photo-title') as HTMLInputElement;
      const fileInput = document.getElementById('photoUpload') as HTMLInputElement;
      const photoKeywordsInput = document.getElementById('photo-keywords') as HTMLInputElement; //optional
      if (photoTitleInput && fileInput.files && fileInput.files.length > 0 && photoKeywordsInput) {
        ...
        if (photoTitle && image) {
          await createPhoto(photoTitle, image, photoKeywords);
 ...
 *
 */
async function getAllPhotosFunc() {
  try {
    const res = await fetch(`http://localhost:8888/home/albums/allPhotos/`, {
      method: 'GET',
      mode: 'cors',
      credentials: 'include',
      headers: {
   ...
   *
   */
  async function searchPhoto(searched_string:string){
    try {
      const res = await fetch(`http://localhost:8888/home/albums/${album_id}/photos/search/${searched_string}`, {
        method: 'GET',
        mode: 'cors',
        credentials: 'include',
        headers: {
   ...
   *
   */
  // for this method we got the help of group 1 (how we can send an image to the server)
  async function createPhoto(title: string, photo: File, photoKeywords: string = "") {
    try {
      ...
      const res = await fetch(`http://localhost:8888/home/albums/${album_id}/photos`, {
        method: 'POST',
        mode: 'cors',
        credentials: 'include',
        headers: {},
      ...
      if (res.ok) {
        ...
        await assignOrUnsignKeywords(photoJson.photo_id, photoKeywords);
   ...
   */
  function addPhotos(photos: Photo[]): void {
  ...
  /**
   * Create a photo card to present the given photo. And add some buttons to the card to delete or edit the photo.
   *
   * @param {string} title - The title of the photo.
   ...
   *
   * @requires
   * - Global variables: photoURL (current url path)
   *
   * @see deletePhoto
   *
   */
  function createPhotoCard(title: string, photo_id: number, album_id: number, creation_date: string = "",  url: string = "https://placehold.co/600x400"): void {
    const container = document.getElementById("photos");
    ...
    editButton.addEventListener('click', async () => {
      const photoTitleInput = document.getElementById('photo-edit-title') as HTMLInputElement;
      const photoCreationDateInput = document.getElementById('photo-creationDate') as HTMLInputElement;
      const hiddenPhotoIdInput = document.getElementById('photo-edit-id') as HTMLInputElement;
      const photoKeywordsInput = document.getElementById('photo-edit-keywords') as HTMLInputElement;
      ...
      if (photoTitleInput && hiddenPhotoIdInput && photoCreationDateInput) {
        ...
        photoKeywordsInput.value = await loadKeywordsFromPhoto(photo_id);
  ...
  async function deletePhoto(photoId:number){
    try {
      const res = await fetch(`http://localhost:8888/home/albums/${album_id}/photos/${photoId}`, {
        method: 'DELETE',
        mode: 'cors',
        credentials: 'include',
        headers: {
   ...
   *
   */
  async function saveChanges(photoTitle:string, photoId:number, photoCreationDate: string, photoKeywords: string){
   try {
     const res = await fetch(`http://localhost:8888/home/albums/${album_id}/photos/${photoId}`, {
       method: 'PATCH',
       mode: 'cors',
     ...
     if (res.ok) {
       ...
       console.log(photoJson);
       await assignOrUnsignKeywords(photoId, photoKeywords).then(() =>
   ...
   *
   */
  async function assignOrUnsignKeywords(photo_id: number, keywordsNew: string) {
    const keywordsOld = await loadKeywordsFromPhoto(photo_id);
    ...
    if (keywordsNew != null && keywordsNew != "") {
      ...
      //for the following 4 lines the help of chatGPT (https://chatgpt.com) was used
      ...
      await assignKeywords(photo_id, addKeywords)
      await unsignKeywords(photo_id, removeKeywords);
    }else {
      await unsignKeywords(photo_id, oldKeywordsArray);
    }
  }
   ...
   *
   */
  async function assignKeywords(photo_id: number, keywordsArray: string[]) {
    //Assign Keywords
    for (const keyword of keywordsArray) {
      try {
        const res = await fetch(`http://localhost:8888/home/albums/${album_id}/photos/${photo_id}/keywords/${keyword}`, {
          method: 'POST',
          mode: 'cors',
          credentials: 'include'
        });
        ...
        if (res.ok) {
          const keywordJson = await res.json();
   ...
   *
   */
  async function unsignKeywords(photo_id: number, keywordsArray: string[]) {
    //Unsign Keywords
    for (const keyword of keywordsArray) {
      try {
        const res = await fetch(`http://localhost:8888/home/albums/${album_id}/photos/${photo_id}/keywords/${keyword}`, {
   ...
   *
   */
  async function loadKeywordsFromAlbum(album_id: number): Promise<string> {
    try {
      const res = await fetch(`http://localhost:8888/home/albums/${album_id}/keywords`, {
        method: 'GET',
        mode: 'cors',
        credentials: 'include',
        headers: {
   ...
   *
   */
  async function loadKeywordsFromPhoto(photo_id: number): Promise<string> {
    try {
