 ...
 * @property {string} imgUrl - The URL of the album's image.
 */
interface Album {
  title: string;
  album_id: number;
...
/**
 * Represents an Keyword List.
 *
 * @interface Keyword
 *
 * @property {string[]} keywords - Store all Keywords
 *
 */
interface Keyword {
  keywords: string[];
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (getAllPhotos) {
    getAllPhotos.addEventListener('click', async () => {
  ...
  if (searchBtn) {
    searchBtn.addEventListener('click', async () => {
      ...
      const albumTitleInput = document.getElementById('searchInputTitle') as HTMLInputElement;
      if (albumTitleInput) {
        const albumTitle = albumTitleInput.value.trim();
        if (albumTitle) {
          const container = document.getElementById("albums");
          container.innerHTML = null;
          await searchAlbum(albumTitle);
        }
      } else {
  ...
  if (saveChangesBtn) {
    saveChangesBtn.addEventListener('click', async () => {
      ...
      const albumTitleInput = document.getElementById('album-edit-title') as HTMLInputElement;
      const hiddenAlbumIdInput = document.getElementById('album-edit-id') as HTMLInputElement;
      const albumKeywordsInput = document.getElementById('album-edit-keywords') as HTMLInputElement;
      if (albumTitleInput && albumKeywordsInput) {
        ...
        if (albumTitle && hiddenAlbumIdInput && albumKeywordsInput) {
          await saveChanges(albumTitle, parseInt(hiddenAlbumIdInput.value), albumKeywords);
        } else {
  ...
  if (addAlbumBtn) {
    addAlbumBtn.addEventListener('click', async () => {
      ...
      const albumTitleInput = document.getElementById('album-title') as HTMLInputElement;
      const albumKeywordsInput = document.getElementById('album-keywords') as HTMLInputElement; //optional
      if (albumTitleInput && albumKeywordsInput) {
        const albumTitle = albumTitleInput.value.trim();
        const albumKeywords = albumKeywordsInput.value.trim();
        // create an album when all necessary inputs are filled
        if (albumTitle) {
          await createAlbum(albumTitle, albumKeywords);
   ...
   *
   */
  async function searchAlbum(searched_string: string) {
    try {
      const res = await fetch(`http://localhost:8888/home/albums/search/${searched_string}`, {
        method: 'GET',
        mode: 'cors',
        credentials: 'include',
        headers: {
   ...
   *
   */
  async function createAlbum(title: string, albumKeywords: string = "") {
    try {
      const res = await fetch(`http://localhost:8888/home/albums`, {
        method: 'POST',
        mode: 'cors',
        credentials: 'include',
        headers: {
      ...
      if (res.ok) {
        ...
        await assignOrUnsignKeywords(albumJson.album_id, albumKeywords);
   ...
   */
  function addAlbums(albums: Album[]): void {
  ...
  /**
   * Creates an album card to present the given album. Adds a delete and edit button to the card.
   *
   ...
   *
   * @requires
   * - Global variables: albumURL (current URL path)
   *
   * @see deleteAlbum
   *
   */
  function createAlbumCard(title: string, album_id: number, imgUrl: string = "https://placehold.co/600x400"): void {
    const container = document.getElementById("albums");
    ...
    editButton.addEventListener('click', async () => {
      ...
      if (albumTitleInput && hiddenAlbumIdInput && albumKeywordsInput) {
        ...
        albumKeywordsInput.value = await loadKeywordsFromAlbum(album_id);
  ...
  async function deleteAlbum(albumId: number) {
    try {
      const res = await fetch(`http://localhost:8888/home/albums/${albumId}`, {
        method: 'DELETE',
        mode: 'cors',
        credentials: 'include',
        headers: {
   ...
   *
   */
  async function saveChanges(albumTitle: string, albumId: number, albumKeywords: string) {
    try {
      const res = await fetch(`http://localhost:8888/home/albums/${albumId}`, {
        method: 'PUT',
        mode: 'cors',
      ...
      if (res.ok) {
        ...
        await assignOrUnsignKeywords(albumId, albumKeywords).then(() =>
   ...
   *
   */
  async function assignOrUnsignKeywords(album_id: number, keywordsNew: string) {
    const keywordsOld = await loadKeywordsFromAlbum(album_id);
    ...
    if (keywordsNew != null && keywordsNew != "") {
      ...
      await assignKeywords(album_id, addKeywords)
      await unsignKeywords(album_id, removeKeywords);
    }else { //when last Keyword should remove
      await unsignKeywords(album_id, oldKeywordsArray);
    }
  }
   ...
   *
   */
  async function assignKeywords(album_id: number, keywordsArray: string[]) {
    //Assign Keywords
    for (const keyword of keywordsArray) {
      try {
        const res = await fetch(`http://localhost:8888/home/albums/${album_id}/keywords/${keyword}`, {
          method: 'POST',
          mode: 'cors',
          credentials: 'include'
        });
   ...
   *
   */
  async function unsignKeywords(album_id: number, keywordsArray: string[]) {
    //Unsign Keywords
    for (const keyword of keywordsArray) {
      try {
        const res = await fetch(`http://localhost:8888/home/albums/${album_id}/keywords/${keyword}`, {
